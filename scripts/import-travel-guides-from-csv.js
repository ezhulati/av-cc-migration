...
/**
 * Import Travel Guides from CSV
 * Imports travel guide articles from WordPress export CSV
 * Converts WordPress blocks to Markdown and creates travel-guides content files
 */
...
const CSV_FILE = './Travel-Advisor-Export-2025-October-17-0730.csv';
const OUTPUT_DIR = './src/content/travel-guides';
const IMAGES_DIR = './public/images/travel-guides';
...
async function importTravelGuides() {
  console.log('🗺️  Importing Travel Guides from CSV...\n');
  ...
  console.log(`✅ Loaded ${records.length} travel guide articles from CSV\n`);
      ...
      for (const imageUrl of imageUrls) {
        if (!imageUrl || downloadedImages.has(imageUrl)) {
          if (imageUrl && downloadedImages.has(imageUrl)) {
            ...
            images.push(`/images/travel-guides/${filename}`);
          }
          continue;
        }
  ...
  console.log(`\n✅ Travel Guides import complete!`);

  console.log('\n💡 Next steps:');
  console.log('  1. Review the generated markdown files in src/content/travel-guides/');
  console.log('  2. Create pages for travel guides');
  console.log('  3. Add travel guides to the site navigation');
}

importTravelGuides().catch(err => {
